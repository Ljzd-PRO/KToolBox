...
命名設定與預設下載位置均屬於個別專案。「命名格式」頁分別儲存目錄結構與命名範本；相對輸出位置以專案為基準，也支援專案外的主機絕對路徑。獨立的「舊下載目錄轉換」分頁可掃描舊位置，並支援暫停、繼續或復原。偵測到舊 dotenv 命名鍵時，WebUI 會先要求確認具有備份的設定遷移，且不會自動掃描目錄。請參閱[命名格式指南](https://ktoolbox.readthedocs.io/latest/zh-Hant/naming/)。
