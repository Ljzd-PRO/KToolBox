...
Настройки имён хранятся в `ktoolbox.toml` проекта и совместно используются CLI и WebUI. Страница **Формат имён** проверяет переменные, сканирует настоящие каталоги загрузок без обращения к Pawchive и до перемещения показывает число произведений, файлов и объём для каждого автора. Преобразование выбрано по умолчанию, но любого автора можно исключить. Новая конфигурация включается только после успешного выполнения всех выбранных операций; при отмене или ошибке выполненные перемещения откатываются в обратном порядке. Подробности см. в [руководстве по формату имён](naming.md).
