...
Настройки имён хранятся в `ktoolbox.toml` проекта и совместно используются CLI и WebUI. Страница **Формат имён** отдельно сохраняет структуру и шаблоны. Отдельная вкладка преобразования без обращения к Pawchive сканирует выбранные пользователем прежние расположения, показывает число работ, файлов и объём для каждого автора и откатывает перемещения при отмене или ошибке. Эти расположения не являются местом сохранения будущих задач. Подробности см. в [руководстве по формату имён](naming.md).
