...
Пауза выполняется совместно: активные сетевые потоки закрываются, готовые и возобновляемые временные файлы остаются, продолжение создаёт новую попытку. Продолжить можно только при паузе, остановке, ошибке или прерывании. Завершённая синхронизация предлагает «Запустить снова» с тем же идентификатором задачи и новой попыткой; завершённая одиночная загрузка этого действия не имеет.
