...
!!! tip "Редактируйте настройки в WebUI"
    Страницы **Глобальные настройки** и **Формат имён** — самый простой способ изменить параметры. Это руководство нужно для ручных файлов, переменных окружения и автоматизации развёртывания; обычную настройку начните с [руководства WebUI](../webui.md).
...
- `ktoolbox.toml` хранит формат имён, список авторов, планы автоматической синхронизации и упорядоченные правила проекта.
...
При создании нового проекта KToolBox определяет IANA-пояс хоста и один раз явно записывает `KTOOLBOX_PUBLISHED_TIME__TARGET_TIMEZONE` в `.env`. Существующий проект без этого ключа не изменяется при чтении конфигурации или запуске WebUI и продолжает использовать значение модели `UTC`. Пояс расписания автоматической синхронизации определяет время запуска, а пояса Service определяют интерпретацию `published` от Pawchive.
...
Рекомендуемая [WebUI](../webui.md) показывает локализованные подписи и описания на всех семи поддерживаемых языках через типизированные элементы управления, а также источник итогового значения, маскирование секретов, редактирование исходного dotenv/TOML, проверку, предварительный просмотр различий и защиту от конфликтов ETag. Английские docstring остаются источником полей и их смысла; для остальных каталогов проверяется полнота всех путей.
...
Каждый проектный документ начинается с `schema_version = 5`. Schema v5 хранит формат имён и планы автоматической синхронизации; прежние расположения используются только инструментом преобразования. `default_output = "downloads"` задаёт общий каталог проекта, а абсолютный путь может указывать за его пределы. См. [руководство по формату имён](../naming.md). Авторы уникальны по `service:id` без учёта регистра; необязательные псевдонимы также уникальны.
