...
- `.env`, `prod.env` и переменные процесса управляют API, передачей и глобальным поведением загрузок.
- `ktoolbox.toml` хранит формат имён, каталоги загрузок, список авторов и упорядоченные правила проекта.
...
Необязательная [WebUI](../webui.md) показывает локализованные подписи и описания на всех семи поддерживаемых языках через типизированные элементы управления, а также источник итогового значения, маскирование секретов, редактирование исходного dotenv/TOML, проверку, предварительный просмотр различий и защиту от конфликтов ETag. Английские docstring остаются источником полей и их смысла; для остальных каталогов проверяется полнота всех путей.

Фиксированные варианты, например уровень журнала, используют Select; поля с рекомендуемыми значениями и допустимым произвольным вводом используют ComboBox. Выбор пути доступен только для настоящих файлов и каталогов, а внутренние имена вроде `attachments` и `external_links.txt` остаются обычным текстом.
...
Каждый проектный документ начинается с `schema_version = 2`. Schema v2 также хранит формат имён и каталоги загрузок проекта; см. [руководство по формату имён](../naming.md). Авторы уникальны по `service:id` без учёта регистра; необязательные псевдонимы также уникальны.
...
Относительные пути вывода и хранилища разрешаются от рабочего каталога.

## Шаблоны имён

Шаблоны и внутренние имена произведений больше не являются глобальными параметрами dotenv. Настройте таблицу `[naming]` проекта на странице WebUI **Формат имён**. После резервного переноса при первом запуске старые ключи отклоняются. Переменные, проверка, сканирование и преобразование описаны в [руководстве по формату имён](../naming.md).
