...
Каждый проектный документ начинается с `schema_version = 4`. Schema v4 хранит формат имён и планы автоматической синхронизации; прежние расположения используются только инструментом преобразования. `default_output = "downloads"` задаёт общий каталог проекта, а абсолютный путь может указывать за его пределы. См. [руководство по формату имён](../naming.md). Авторы уникальны по `service:id` без учёта регистра; необязательные псевдонимы также уникальны.
