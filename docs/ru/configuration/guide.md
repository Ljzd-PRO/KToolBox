...
Каждый проектный документ начинается с `schema_version = 4`. Schema v4 хранит формат имён и планы автоматической синхронизации; прежние расположения используются только инструментом преобразования. См. [руководство по формату имён](../naming.md). Авторы уникальны по `service:id` без учёта регистра; необязательные псевдонимы также уникальны.
