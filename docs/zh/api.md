...
OpenAPI 中受 `cookieAuth` 保护的 5 个操作明确排除：列出账号收藏，以及添加或删除作品、作者收藏。作品标记属于公开契约，不使用账号会话。

## 参数校验

- `service`、作者 ID 和作品 ID 去除空白后必须非空。
- 提供搜索词时，长度至少为 3 个字符。
...
| `PawchiveConflictError` | HTTP `409`，包括作品已经标记 |
...
- 阅读[关于 Pawchive](about-pawchive.md)，了解后端范围与隐私边界。
- 通过 [MCP 指南](mcp.md)配置 WebUI 的认证工具访问。
- 面向最终用户的支持流程请参阅[命令指南](commands/guide.md)。
