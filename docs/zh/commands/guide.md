...
!!! tip "推荐使用 WebUI"
    普通下载与同步建议从 [WebUI](../webui.md) 开始。只有需要脚本、Shell 自动化或纯终端工作流时，才需要使用命令行。
...
未提供 `--output` 时，KToolBox 使用项目默认下载位置。仅需覆盖本次运行时再传入 `--output`；允许使用项目外的绝对路径。
...
安装 `ktoolbox[webui]` 后，可将面板绑定到项目目录。可以预先配置账户，也可以使用本次运行时终端输出的 `admin` 用户名和随机密码。若缺少 `ktoolbox.toml`，程序会在终端提示警告并自动创建：
