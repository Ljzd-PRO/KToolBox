...
命名设置保存在项目 `ktoolbox.toml` 中，由 CLI 与 WebUI 下载共同使用。“命名格式”页会校验模板变量，在不访问 Pawchive 的情况下扫描真实下载根目录，并在移动任何内容前展示每位作者的作品、文件和大小统计。默认勾选转换，但可以取消任意作者；只有全部选中移动成功后才启用新配置，取消或失败会逆序回滚已完成操作。模板、迁移和恢复细节详见[命名格式指南](naming.md)。
