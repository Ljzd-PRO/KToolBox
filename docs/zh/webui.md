...
- **任务：** 创建、编辑、暂停、恢复、停止、重新执行、删除并查看同步或单篇下载；点击整行可进入详情，也可选择多项执行兼容的批量操作。
...
暂停采用协作方式：关闭活动网络流，保留已完成文件和可续传临时文件；恢复会新建一次尝试。停止会保留任务定义，以便编辑后重新执行。只有暂停、停止、失败和中断任务可以恢复；已完成任务仍可编辑和删除，但不能恢复。服务进程重启后，原运行任务标记为 `interrupted`，残留实时进度会被清空，并且必须由用户明确恢复。
