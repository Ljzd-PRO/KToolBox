...
命名设置保存在项目 `ktoolbox.toml` 中，由 CLI 与 WebUI 下载共同使用。“命名格式”页分别保存目录结构和模板；独立的旧目录转换标签页会在不访问 Pawchive 的情况下扫描用户选择的旧位置，展示每位作者的作品、文件和大小统计，并在取消或失败时逆序回滚。旧位置不会成为未来任务的下载目录。模板、首次启动决策和恢复细节详见[命名格式指南](naming.md)。
