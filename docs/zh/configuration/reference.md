...
| 字段 | 类型 | 默认值 | 说明 |
| --- | --- | --- | --- |
| `schema_version` | 字面量 `5` | `5` | 项目 Schema 版本；旧版文档会在保存时升级。 |
| `creators` | 表数组 | 空 | 已保存作者清单。 |
| `blockers` | 表数组 | 空 | 有序忽略规则定义。 |
| `default_output` | 字符串 | `downloads` | 默认下载目录。相对路径从项目根目录解析；绝对路径可指向项目外。 |
| `naming` | 表 | 项目默认值 | 目录布局与文件名模板；旧下载位置仅在转换工具中填写。详见[命名格式指南](../naming.md)。 |
| `automatic_sync` | 表数组 | 空 | 定时作者同步计划。详见[自动同步指南](../automatic-sync.md)。 |

### 作者项

| 字段 | 类型 | 默认值 | 说明 |
| --- | --- | --- | --- |
| `service` | 非空字符串 | 必填 | Pawchive 平台。 |
...
| `type` | 字符串 | `field-match` | 已注册忽略规则实现，未知类型会被拒绝。 |
...
| `username` | 字符串 | 空 | 可选用户名；留空时启动过程使用 `admin`。 |
| `password_hash` | 秘密字符串 | 空 | 推荐的固定 Argon2id 密码哈希。 |
| `password` | 秘密字符串 | 空 | 明文后备值；两种密码均留空时由启动过程生成。 |
