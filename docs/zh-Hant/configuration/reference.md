...
| 欄位 | 型別 | 預設值 | 說明 |
| --- | --- | --- | --- |
| `schema_version` | 常值 `5` | `5` | 專案 Schema 版本；舊版文件會在儲存時升級。 |
| `creators` | 表格陣列 | 空 | 已儲存的創作者清單。 |
| `blockers` | 表格陣列 | 空 | 有順序的忽略規則規格。 |
| `default_output` | 字串 | `downloads` | 預設下載目錄。相對路徑從專案根目錄解析；絕對路徑可指向專案外。 |
| `naming` | 表格 | 專案預設值 | 目錄配置與檔名範本；舊下載位置只在轉換工具中填寫。請參閱[命名格式指南](../naming.md)。 |
| `automatic_sync` | 表格陣列 | 空 | 定時創作者同步計畫。請參閱[自動同步指南](../automatic-sync.md)。 |

### 創作者項目

| 欄位 | 型別 | 預設值 | 說明 |
| --- | --- | --- | --- |
| `service` | 非空字串 | 必填 | Pawchive 平台。 |
...
| `username` | 字串 | 空 | 可選使用者名稱；留空時啟動程序使用 `admin`。 |
| `password_hash` | 機密字串 | 空 | 建議使用的固定 Argon2id 密碼雜湊。 |
| `password` | 機密字串 | 空 | 純文字備用值；兩種密碼均留空時由啟動程序產生。 |
