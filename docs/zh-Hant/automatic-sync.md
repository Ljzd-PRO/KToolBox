...
每次執行開始前會固定截止時間。KToolBox 優先使用 Pawchive 具有 UTC 語意的 `added` 時間，從上次成功檢查點向前重疊 24 小時，並依平台、作者 ID 與作品 ID 去重。
...
- **新增作品**卡片同時控制統計總數與下方清單的週期：**當天**從本地零點開始計算，**近 1 天**、7 天、14 天和 30 天均為滾動時間視窗。
...
- 透過 [WebUI 指南](webui.md)完成安裝、驗證與任務監控。
- 調整下載並行數前請先查看[設定指南](configuration/guide.md)。
- 首次無人值守執行前請確認[命名格式](naming.md)。
