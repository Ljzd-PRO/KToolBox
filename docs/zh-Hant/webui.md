...
命名設定儲存在專案 `ktoolbox.toml`，CLI 與 WebUI 下載共用同一設定。「命名格式」頁會驗證範本變數，在不連線 Pawchive 的情況下掃描實際下載根目錄，並於移動前顯示每位創作者的作品、檔案與容量統計。預設勾選轉換，但可取消任一創作者；只有全部選取項目移動成功後才啟用新設定，取消或失敗會反向復原已完成操作。範本、遷移與復原細節請參閱[命名格式指南](naming.md)。
