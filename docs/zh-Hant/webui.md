...
命名設定與預設輸出位置儲存在專案 `ktoolbox.toml`，由 CLI、WebUI、MCP、作品下載與自動同步共用。「命名格式」頁分別儲存目錄結構與範本；獨立的舊目錄轉換分頁只掃描使用者明確選取的位置，不連線 Pawchive，並支援暫停、繼續或復原。舊位置絕不會成為未來工作的下載目錄。繼承規則、舊設定遷移精靈與復原細節請參閱[命名格式指南](naming.md)。
