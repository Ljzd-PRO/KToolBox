...
命名設定儲存在專案 `ktoolbox.toml`，CLI 與 WebUI 下載共用同一設定。「命名格式」頁分別儲存目錄結構與範本；獨立的舊目錄轉換分頁會在不連線 Pawchive 的情況下掃描使用者選取的舊位置，顯示每位創作者的作品、檔案與容量統計，並於取消或失敗時反向復原。舊位置不會成為未來工作的下載目錄。範本、首次啟動決策與復原細節請參閱[命名格式指南](naming.md)。
