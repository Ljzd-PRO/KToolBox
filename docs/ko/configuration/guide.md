...
모든 프로젝트 문서는 `schema_version = 5`로 시작합니다. Schema v5는 프로젝트 이름 형식과 자동 동기화 계획을 저장하며, 이전 다운로드 위치는 이름 변환 도구에서만 사용합니다. `default_output = "downloads"`는 프로젝트 공통 기본값이며 절대 경로로 프로젝트 밖도 지정할 수 있습니다. [이름 형식 가이드](../naming.md)를 참조하세요. 크리에이터는 대소문자를 구분하지 않는 `service:id`로 고유하며 선택적 별칭도 고유합니다.
...
상대 출력 및 버킷 경로는 작업 디렉터리에서 확인됩니다.

## 이름 지정 템플릿

이름 템플릿과 작품 내부 이름은 더 이상 전역 dotenv 설정이 아닙니다. WebUI의 **이름 형식** 페이지에서 프로젝트 `[naming]`을 설정하세요. 최초 시작의 백업 마이그레이션 뒤에는 이전 이름 키가 거부됩니다. 변수, 검증, 스캔과 변환은 [이름 형식 가이드](../naming.md)를 참조하세요.
