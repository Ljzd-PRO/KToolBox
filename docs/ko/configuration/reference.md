...
| 필드 | 형식 | 기본값 | 설명 |
| --- | --- | --- | --- |
| `schema_version` | 리터럴 `5` | `5` | 프로젝트 스키마 버전. 이전 문서는 저장할 때 업그레이드됩니다. |
| `creators` | 테이블 배열 | 비어 있음 | 저장된 크리에이터 목록. |
| `blockers` | 테이블 배열 | 비어 있음 | 순서가 있는 제외 규칙 사양. |
| `default_output` | 문자열 | `downloads` | 기본 다운로드 디렉터리입니다. 상대 경로는 프로젝트 루트에서 해석하며 절대 경로는 프로젝트 밖을 가리킬 수 있습니다. |
| `naming` | 테이블 | 프로젝트 기본값 | 디렉터리 구조와 파일 이름 템플릿. 이전 위치는 변환 도구에서만 입력합니다. [이름 형식 가이드](../naming.md) 참조. |
| `automatic_sync` | 테이블 배열 | 비어 있음 | 예약된 크리에이터 동기화 계획. [자동 동기화 가이드](../automatic-sync.md) 참조. |
