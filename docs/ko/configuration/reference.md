...
이 설정은 `ktoolbox[webui]`를 설치한 경우에만 필요합니다. 기본 자격 증명은 없으며 사용자 이름과 두 가지 암호 형식 중 하나가 없으면 서버가 시작을 거부합니다.
...
| `username` | 문자열 | 비어 있음 | 단일 계정에 필요한 사용자 이름. |
| `password_hash` | 비밀 문자열 | 비어 있음 | 권장 Argon2id 암호 해시. |
| `password` | 비밀 문자열 | 비어 있음 | 일반 텍스트 대체 값. `password_hash` 설정 시 무시. |
