...
`ktoolbox[webui]`를 설치하고 프로젝트 디렉터리를 전달한 다음 `KTOOLBOX_WEBUI__USERNAME`과 `KTOOLBOX_WEBUI__PASSWORD_HASH` 또는 `KTOOLBOX_WEBUI__PASSWORD`를 설정하세요. `ktoolbox.toml`이 없으면 경고 후 자동 생성됩니다. 유효한 Argon2 해시가 권장됩니다. 프로젝트 잠금은 같은 프로젝트의 두 번째 WebUI 프로세스도 거부합니다.
