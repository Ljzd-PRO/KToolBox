...
네. **이름 형식 > 디렉터리 구조**에서 첨부 파일 디렉터리를 `.` 또는 `./`로 설정하세요. 해당 프로젝트 설정은 다음과 같습니다.
...
첨부 파일 이름이 기본 파일(표지), `post.json` 또는 다른 메타데이터와 겹치지 않도록 하세요. 이전 `KTOOLBOX_JOB__POST_STRUCTURE__ATTACHMENTS=./` 설정으로 받은 파일은 **이전 다운로드 변환 > 설정 붙여넣기**에서 변환할 수 있습니다. 미리보기와 변환 절차는 [이름 형식 가이드](naming.md)를 참고하세요. 다른 내부 경로에는 여전히 안전하고 비어 있지 않은 상대 이름이 필요합니다.
...
## 동기화가 실패한 실제 원인을 어떻게 확인하나요?

실패한 작업을 열고 **작업 실패 원인**을 확인하세요. KToolBox는 관련 크리에이터 또는 파일, 실패 단계, 재시도 가능성 및 권장 조치를 표시합니다. 응답 형식 비호환은 Pawchive가 필드 형태를 변경했음을 의미할 수 있으므로 KToolBox를 업데이트하고 패널에 표시되는 안전한 작업 이름과 필드 경로를 보고하세요. 네트워크, 시간 초과, 속도 제한 오류는 재시도로 해결될 수 있습니다. 진단 보고서에는 업스트림 응답 본문, 작품 제목, Cookie 또는 전체 다운로드 URL이 포함되지 않습니다.
