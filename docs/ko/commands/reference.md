...
이 명령을 사용하기 전에 `ktoolbox[webui]`를 설치하세요. 기본 명령은 프로젝트 디렉터리를 받고 `ktoolbox.toml`이 없으면 경고 후 생성합니다. 계정 미설정 시 해당 프로세스용 `admin`과 무작위 암호를 터미널에 출력하며, WebUI 설정으로 고정 자격 증명을 지정할 수 있습니다.
