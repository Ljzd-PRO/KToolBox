...
**디렉터리 구조**에서 첨부 파일 디렉터리를 `.` 또는 `./`로 설정하면 표지 및 메타데이터와 같은 위치에 저장됩니다. 이 예외는 첨부 파일 디렉터리에만 적용됩니다. 본문, 외부 링크 파일 및 수정본 디렉터리에는 여전히 이름이 필요합니다.

다음 v0 설정으로 받은 파일은 **이전 다운로드 변환 > 설정 붙여넣기**에서 변환할 수 있습니다.

```dotenv
KTOOLBOX_JOB__POST_STRUCTURE__ATTACHMENTS=./
```

생략한 ENV 필드에는 v0 기본값이 적용되므로 v0에서 변경한 다른 이름 설정도 함께 붙여 넣으세요. 변환 대상은 항상 현재 프로젝트 형식입니다. 확인 전에 미리보기를 검토하세요. `post.json`이나 크리에이터 인덱스로 식별한 첨부 파일은 수정본의 파일까지 개별적으로 이동합니다. 표지와 메타데이터는 첨부 파일로 취급하지 않으며, 식별하지 못한 파일은 작품 디렉터리 안의 기존 상대 위치를 유지합니다. 대상 파일이 이미 있거나 경로가 안전하지 않으면 덮어쓰지 않고 변환을 차단합니다.
...
저장된 각 레이아웃에는 당시의 게시 시간 정책도 고정됩니다. 수정 전 프로젝트 버전은 “Pawchive 원본 값 사용”으로 표시됩니다. 붙여 넣은 v0 `.env`는 기본적으로 “Kemono UTC”를 사용하고, TOML은 Kemono UTC, Pawchive 원본 값 또는 사용자 지정 Service 시간대를 선택할 수 있습니다. 예를 들어 Fanbox `2025-12-21T00:35:43`은 `Asia/Tokyo`로 해석되고 UTC 대상에서는 `2025-12-20T15:35:43+00:00`이 되므로 `{published}` 경로가 `2025-12-21`에서 `2025-12-20`으로 이동합니다. `post.json`의 Pawchive 원본 값은 유지됩니다.
...
- 이름 설정 페이지와 변환 절차는 [WebUI 가이드](webui.md)를 참조하세요.
- 전역 네트워크와 다운로더 설정은 [설정 가이드](configuration/guide.md)를 사용하세요.
- 기존 프로젝트를 업그레이드할 때는 [v1 마이그레이션 가이드](migration-v1.md)를 따르세요.
