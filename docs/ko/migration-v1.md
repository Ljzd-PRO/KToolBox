...
이름 템플릿과 기본 출력은 프로젝트 Schema v5에 속하며 이전 다운로드 위치는 변환 도구에서만 사용합니다. 이전 이름 키가 감지되면 WebUI 시작 시에는 경고만 표시하고, 로그인 후 사용자가 필드별 차이와 백업된 원자적 마이그레이션을 명시적으로 확인한 뒤에만 파일을 변경합니다. 디렉터리 스캔은 별도의 명시적 단계입니다. 자세한 내용은 [이름 형식 가이드](naming.md)를 참조하세요.
