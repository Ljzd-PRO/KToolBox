...
`ktoolbox[webui]` をインストールし、プロジェクトディレクトリを渡し、`KTOOLBOX_WEBUI__USERNAME` と `KTOOLBOX_WEBUI__PASSWORD_HASH` または `KTOOLBOX_WEBUI__PASSWORD` を設定します。`ktoolbox.toml` がなければ警告後に自動作成されます。ハッシュが推奨で、有効な Argon2 値が必要です。プロジェクトロックは同じプロジェクトの 2 つ目の WebUI プロセスも拒否します。
