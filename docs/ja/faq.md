...
はい。「命名形式 → ディレクトリ構造」で添付ファイルのディレクトリを `.` または `./` に設定します。対応するプロジェクト設定は次のとおりです。
...
添付ファイル名がメインファイル（カバー）、`post.json` などのメタデータと重複しないようにしてください。旧設定 `KTOOLBOX_JOB__POST_STRUCTURE__ATTACHMENTS=./` で保存したファイルは「旧ダウンロード変換 → 設定を貼り付け」から変換できます。プレビューと変換の手順は[命名ガイド](naming.md)を参照してください。他の内部パスには引き続き安全で空でない相対名が必要です。
