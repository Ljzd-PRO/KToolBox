...
1. 古い設定とダウンロード先をバックアップします。
2. 既存のプロジェクト用ディレクトリで [WebUI](webui.md) をインストールして起動します。
3. 移行ダイアログで検出された古い設定を項目ごとに確認してから変更を承認します。
4. ディレクトリ変換をプレビューし、スケジュールや完全な保存を有効にする前に小規模な同期を実行します。

WebUI はバックアップと競合確認を行い、設定移行と任意のディレクトリ変換を分けて扱います。以下の詳細表は主にスクリプトと Python 連携向けです。
...
命名テンプレートと既定出力はプロジェクト Schema v5 に移り、以前のダウンロード場所は変換ツールだけで扱います。旧命名キーを検出した場合、WebUI の起動時は警告だけを表示し、ログイン後に項目ごとの差分とバックアップ付き原子的移行を明示確認してからファイルを変更します。ディレクトリのスキャンは別の明示操作です。詳細は[命名形式ガイド](naming.md)を参照してください。
