...
命名テンプレートはプロジェクト Schema v4 に移り、以前のダウンロード場所は命名設定ではなく変換ツールだけで扱います。WebUI の初回起動時に `.env` と `prod.env` をバックアップし、旧命名キーを `ktoolbox.toml` へ移行して削除したうえで、既存内容を無視するか変換を確認するかを選択します。詳細は[命名形式ガイド](naming.md)を参照してください。
