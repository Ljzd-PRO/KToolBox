...
命名テンプレートと既定出力はプロジェクト Schema v4 に移り、以前のダウンロード場所は変換ツールだけで扱います。旧命名キーを検出した場合、WebUI の起動時は警告だけを表示し、ログイン後に項目ごとの差分とバックアップ付き原子的移行を明示確認してからファイルを変更します。ディレクトリのスキャンは別の明示操作です。詳細は[命名形式ガイド](naming.md)を参照してください。
