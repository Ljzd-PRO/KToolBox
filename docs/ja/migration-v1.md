...
v1 には新しい HeroUI パネルが追加されます。過去の実験的な `webui` ブランチを移行または再利用するものではありません。`ktoolbox[webui]` をインストールし、プロジェクトディレクトリを選び、新しい単一アカウントを設定してください。`ktoolbox.toml` がない場合は警告後に自動作成されますが、既定の認証情報は作成されません。
