...
命名テンプレートとダウンロードルートはプロジェクト Schema v2 に移りました。WebUI の初回起動時に `.env` と `prod.env` をバックアップし、旧命名キーを `ktoolbox.toml` へ移行して削除し、1 回限りの通知を表示します。既存ダウンロードを変換する前に生成されるレイアウトを確認してください。詳細は[命名形式ガイド](naming.md)を参照してください。
