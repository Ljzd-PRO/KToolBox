...
| フィールド | 型 | 既定値 | 説明 |
| --- | --- | --- | --- |
| `schema_version` | リテラル `5` | `5` | プロジェクトスキーマバージョン。旧文書は保存時に更新されます。 |
| `creators` | テーブル配列 | 空 | 保存済みクリエイター一覧。 |
| `blockers` | テーブル配列 | 空 | 順序付き除外ルール仕様。 |
| `default_output` | 文字列 | `downloads` | 既定のダウンロード先。相対パスはプロジェクトルート基準で、絶対パスはプロジェクト外も指定できます。 |
| `naming` | テーブル | プロジェクト既定値 | ディレクトリ構成とファイル名テンプレート。以前の場所は変換ツールだけで入力します。[命名形式ガイド](../naming.md)を参照。 |
| `automatic_sync` | テーブル配列 | 空 | 定期クリエイター同期プラン。[自動同期ガイド](../automatic-sync.md)を参照。 |
