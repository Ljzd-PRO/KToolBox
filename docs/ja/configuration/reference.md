...
これらの設定は `ktoolbox[webui]` をインストールした場合だけ必要です。既定の認証情報はなく、ユーザー名といずれかの形式のパスワードがない場合はサーバーが起動を拒否します。
...
| `username` | 文字列 | 空 | 単一アカウントに必要なユーザー名。 |
| `password_hash` | シークレット文字列 | 空 | 推奨する Argon2id パスワードハッシュ。 |
| `password` | シークレット文字列 | 空 | 平文の代替値。`password_hash` 設定時は無視。 |
