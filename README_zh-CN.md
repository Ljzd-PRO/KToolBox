...
命名设置和默认下载位置均为项目专属。“命名格式”页分别保存目录结构和命名模板；相对输出位置以项目为基准，也可使用项目外的主机绝对路径。独立的“旧下载目录转换”标签页负责扫描旧位置，并支持暂停、继续或回滚。检测到旧 dotenv 命名键时，WebUI 会先要求用户确认带备份的配置迁移，且不会自动扫描目录。详见[命名格式指南](https://ktoolbox.readthedocs.io/latest/zh/naming/)。
