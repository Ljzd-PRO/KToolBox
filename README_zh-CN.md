...
>
> 升级前请阅读 [v1.0.0 发布说明](https://github.com/Ljzd-PRO/KToolBox/releases/tag/v1.0.0)和文档站中的[迁移指南](https://ktoolbox.readthedocs.io/latest/zh/migration-v1/)。

**文档导航：** [开始使用](https://ktoolbox.readthedocs.io/latest/zh/) · [命令行](https://ktoolbox.readthedocs.io/latest/zh/commands/guide/) · [WebUI](https://ktoolbox.readthedocs.io/latest/zh/webui/) · [版本迁移](https://ktoolbox.readthedocs.io/latest/zh/migration-v1/) · [问题排查](https://ktoolbox.readthedocs.io/latest/zh/faq/)
...
- 通过支持七种语言的响应式 HeroUI 面板管理单个同步项目，提供持久任务、实时进度、配置表单、作者与忽略规则编辑以及深浅色主题。
...
![KToolBox 全局配置](docs/assets/webui/30-global-configuration-log-level-light.png)

完整界面支持简体中文、繁体中文、英语、日语、韩语、法语和俄语。首次访问跟随浏览器语言；手动选择会被持久保存，并同时切换日期、数字、排序、配置说明、表单校验和服务端错误消息。
...
命名设置和默认下载位置均为项目专属。“命名格式”页分别保存目录结构和命名模板；相对输出位置以项目为基准，也可使用项目外的主机绝对路径。可随时使用的“旧下载目录转换”支持多选项目历史版本，或在高亮编辑器中粘贴旧 `.env`/TOML；当前项目格式始终是只读目标，粘贴原文不会持久化。扫描后的移动支持暂停、继续或回滚。检测到旧 dotenv 命名键时，WebUI 会先要求用户确认带备份的配置迁移，且不会自动扫描目录。详见[命名格式指南](https://ktoolbox.readthedocs.io/latest/zh/naming/)。
