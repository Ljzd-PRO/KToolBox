...
이름 설정은 프로젝트별로 저장됩니다. **이름 형식** 페이지에서 디렉터리 구조와 템플릿을 각각 저장하며, 별도의 이전 다운로드 변환 탭에서 이전 위치를 스캔하고 완전한 롤백과 함께 선택한 크리에이터를 변환할 수 있습니다. WebUI 최초 시작 시 이전 전역 값을 마이그레이션하고 기존 콘텐츠를 무시할지 변환을 검토할지 선택합니다. [이름 형식 가이드](https://ktoolbox.readthedocs.io/latest/ko/naming/)를 참조하세요.
