...
이름 설정과 기본 다운로드 위치는 프로젝트별로 저장됩니다. **이름 형식** 페이지에서 디렉터리 구조와 템플릿을 각각 저장하고, 상대 출력은 프로젝트 기준으로 해석하며 프로젝트 밖의 절대 경로도 지원합니다. 별도의 이전 다운로드 변환 탭은 일시 중지, 계속 및 롤백을 지원합니다. 이전 dotenv 이름 키가 감지되면 WebUI는 백업된 설정 마이그레이션을 먼저 확인하고 디렉터리를 자동 스캔하지 않습니다. [이름 형식 가이드](https://ktoolbox.readthedocs.io/latest/ko/naming/)를 참조하세요.
