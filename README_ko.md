...
이름 설정은 프로젝트별로 저장됩니다. **이름 형식** 페이지는 템플릿을 검증하고 디렉터리 트리를 미리 보여 주며 실제 다운로드 루트를 스캔합니다. 선택한 크리에이터는 원자적 적용과 완전한 롤백을 보장하며 변환할 수 있습니다. 이전 전역 이름 값은 WebUI를 처음 시작할 때 백업 후 마이그레이션됩니다. [이름 형식 가이드](https://ktoolbox.readthedocs.io/latest/ko/naming/)를 참조하세요.
...
`.env`는 런타임과 전송 동작을 제어하고, 프로젝트 수준 `ktoolbox.toml`은 이름 형식, 다운로드 루트, 크리에이터 목록과 제외 규칙을 저장합니다.
