...
Формат имён задаётся для каждого проекта. Страница **Формат имён** отдельно сохраняет структуру и шаблоны, а отдельная вкладка преобразования старых загрузок сканирует прежние расположения и перемещает выбранных авторов с полным откатом. При первом запуске WebUI переносит старые глобальные значения и предлагает игнорировать прежние данные или проверить преобразование. См. [руководство по формату имён](https://ktoolbox.readthedocs.io/latest/ru/naming/).
