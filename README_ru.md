...
Формат имён теперь задаётся для каждого проекта. Страница **Формат имён** проверяет шаблоны, показывает дерево каталогов, сканирует настоящие каталоги загрузок и преобразует выбранных авторов с атомарным включением и полным откатом. Старые глобальные значения сохраняются в резервной копии и переносятся при первом запуске WebUI. См. [руководство по формату имён](https://ktoolbox.readthedocs.io/latest/ru/naming/).
...
Файл `.env` управляет выполнением и передачей данных. Проектный `ktoolbox.toml` хранит формат имён, каталоги загрузок, список авторов и правила исключения:
