...
MODEL_SECTIONS = {"api", "downloader", "job.post_structure", "job", "webui", "logger", "published_time"}
LEGACY_PROJECT_NAMING_FIELDS = {
    "job.post_dirname_format",
    "job.post_structure.attachments",
    "job.post_structure.content",
    "job.post_structure.external_links",
    "job.post_structure.file",
    "job.post_structure.revisions",
    "job.mix_posts",
    "job.sequential_filename",
    "job.sequential_filename_excludes",
    "job.filename_format",
    "job.group_by_year",
    "job.group_by_month",
    "job.year_dirname_format",
    "job.month_dirname_format",
}

# fmt: off
SECTION_LABELS = {
    "zh-Hant": {"api": "Pawchive API", "downloader": "檔案下載", "job": "下載工作", "logger": "日誌", "published_time": "發布時間", "webui": "WebUI", "general": "一般"},
    "ja": {"api": "Pawchive API", "downloader": "ファイルダウンロード", "job": "ダウンロードジョブ", "logger": "ログ", "published_time": "公開日時", "webui": "WebUI", "general": "一般"},
    "ko": {"api": "Pawchive API", "downloader": "파일 다운로드", "job": "다운로드 작업", "logger": "로그", "published_time": "게시 시간", "webui": "WebUI", "general": "일반"},
    "fr": {"api": "API Pawchive", "downloader": "Téléchargements", "job": "Tâches de téléchargement", "logger": "Journalisation", "published_time": "Date de publication", "webui": "WebUI", "general": "Général"},
    "ru": {"api": "API Pawchive", "downloader": "Загрузка файлов", "job": "Задачи загрузки", "logger": "Журналирование", "published_time": "Время публикации", "webui": "WebUI", "general": "Общие"},
}
...
LABELS = {
    "zh-Hant": {
        ...
        "published_time.target_timezone": "目標時區", "published_time.fallback_service_timezone": "未設定 Service 的時區", "published_time.service_timezones": "Service 時區",
    ...
    "ja": {
        ...
        "published_time.target_timezone": "対象タイムゾーン", "published_time.fallback_service_timezone": "未設定 Service のタイムゾーン", "published_time.service_timezones": "Service タイムゾーン",
    ...
    "ko": {
        ...
        "published_time.target_timezone": "대상 시간대", "published_time.fallback_service_timezone": "미설정 Service 시간대", "published_time.service_timezones": "Service 시간대",
    ...
    "fr": {
        ...
        "published_time.target_timezone": "Fuseau horaire cible", "published_time.fallback_service_timezone": "Fuseau des Service non configurés", "published_time.service_timezones": "Fuseaux horaires des Service",
    ...
    "ru": {
        ...
        "published_time.target_timezone": "Целевой часовой пояс", "published_time.fallback_service_timezone": "Пояс для Service без настройки", "published_time.service_timezones": "Часовые пояса Service",
...
def render() -> str:
    catalogs = {
        locale: {
            ...
            "labels": {
                path: label for path, label in LABELS[locale].items() if path not in LEGACY_PROJECT_NAMING_FIELDS
            },
    ...
    missing = {
        locale: sorted(set(catalogs[locale]["labels"]) - set(catalogs[locale]["descriptions"])) for locale in LOCALES
    }
