...
命名設定はプロジェクト固有になりました。「命名形式」ページではテンプレート検証、ディレクトリツリーのプレビュー、実際のダウンロードルートのスキャンを行い、選択したクリエイターをアトミックな有効化と完全なロールバック付きで変換できます。旧グローバル命名値は WebUI 初回起動時にバックアップして移行します。[命名形式ガイド](https://ktoolbox.readthedocs.io/latest/ja/naming/)を参照してください。
...
`.env` はランタイムと転送動作を制御し、プロジェクト単位の `ktoolbox.toml` は命名形式、ダウンロードルート、クリエイター一覧、除外ルールを保存します。
