...
命名設定はプロジェクト固有です。「命名形式」ページではディレクトリ構造とテンプレートを個別に保存し、独立した以前のダウンロード変換タブで旧場所をスキャンして、完全なロールバック付きで選択したクリエイターを変換できます。WebUI 初回起動時は旧グローバル値を移行し、既存内容を無視するか変換を確認するかを選択します。[命名形式ガイド](https://ktoolbox.readthedocs.io/latest/ja/naming/)を参照してください。
